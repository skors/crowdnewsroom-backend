...
export default class MultiSelectPicker extends Component {
  ...
  componentDidMount() {
    const itemPromise = authorizedFetch(
      `/forms/investigations/${this.props.investigationSlug}/${
        this.props.property
      }`
    )

    const responsePromise = authorizedFetch(
      `/forms/responses/${this.props.responseId}`
    )

    Promise.all([itemPromise, responsePromise]).then(([items, response]) => {
      const selectedItems = items.filter(item =>
        response[this.props.property].includes(item.id)
      )
  ...
  update(updatedResponse) {
    const selectedItems = this.state.items.filter(item =>
      updatedResponse[this.props.property].includes(item.id)
    )
  ...
  updateSelection({ selectedItems }) {
    authorizedPATCH(`/forms/responses/${this.props.responseId}`, {
      body: JSON.stringify({
        [this.props.property]: selectedItems.map(item => item.id)
      })
    }).then(this.update)
  }
      ...
      <div>
        ...
        <ul>
          {this.state.selectedItems.map(item => (
            <li key={item.id}>
              <div className="bx--tag bx--tag--beta">
