    ...
    <TableContainer>
      <TableToolbar>
        <TableBatchActions {...getBatchActionProps()}>
          <TableBatchAction
            onClick={() => removeUsers(selectedRows)}
            icon="close"
      ...
      <Table>
        ...
        <TableBody>
          {rows.map(row => (
            <TableRow key={row.id} {...getRowProps({ row, id: row.id })}>
              <TableSelectRow {...getSelectionProps({ row })} />
              {row.cells.map(cell => (
                <TableCell key={cell.id}>{cell.value}</TableCell>
              ))}
              <ManageRoleCell
                row={keyedRows[row.id]}
                availableRoles={availableRoles}
  ...
  render() {
    return (
      <Form onSubmit={this.submit}>
        ...
        <p>
          {gettext(
            'Enter the email of the collaborator you would like to invite.'
          )}
        </p>
      </Form>
    )
...
function removeUser(user, investigationSlug) {
  return authorizedDELETE(
    `/forms/investigations/${investigationSlug}/groups/${user.role}/users/${
      user.id
    }`
  )
}
...
export default class InvestigationUsers extends Component {
  ...
  componentDidMount() {
    ...
    Promise.all([userPromise, invitationPromise]).then(() => {
  ...
  inviteUser(email) {
    ...
    authorizedPOST(`/forms/investigations/${this.state.slug}/invitations`, {
  ...
  loadUsers() {
    return authorizedFetch(
      `/forms/investigations/${this.state.slug}/users`
    ).then(json => {
  ...
  loadInvitations() {
    return authorizedFetch(
      `/forms/investigations/${this.state.slug}/invitations`
    ).then(invitations => {
      this.setState({
  ...
  updateUserRole(userID, role) {
    ...
    authorizedPOST(
      `/forms/investigations/${this.state.slug}/groups/${role}/users`,
  ...
  removeUsers(users) {
    ...
    const selectedUsers = this.state.users.filter(
      user => userEmails.indexOf(user.email) !== -1
    )
    const selectedInvitations = this.state.invitations.filter(
      user => userEmails.indexOf(user.email) !== -1
    )

    const removeUserPromises = selectedUsers.map(user =>
      removeUser(user, this.state.slug)
    )
    Promise.all(removeUserPromises).then(this.loadUsers)

    const removeInvitationPromises = selectedInvitations.map(invitation =>
  ...
  render() {
    ...
    const headers = [
      { key: 'first_name', header: gettext('First name') },
      { key: 'last_name', header: gettext('Last Name') },
      { key: 'email', header: gettext('E-Mail') }
    ]
    return (
      <div className="investigation-users">
        <p className="investigation-users--section">
          {gettext(
            'Invite members of your team to help contribute to your investigation by assigning them specific permissions.'
          )}
          {gettext(
            'Each collaborator will be able to access your project through their own Crowdnewsroom account.'
          )}
          {gettext(
            "As the creator, you remain ultimately responsible for your project and for your team's interactions with contributors."
          )}
          {gettext(
            'Pick collaborators that you trust and that will do a good job working with your community.'
          )}
        ...
        <div className="investigation-users--section">
          ...
          <InviteUser
            inviteCallback={this.inviteUser}
        ...
        <div className="cnr--datatable-overflowable investigation-users--section">
          ...
          <p>
            <dl className="roles-list">
              ...
              <dd className="roles-list--dd">
                {gettext(
                  'An admin can manage the investigation settings page and the collaborators'
                )}
              </dd>
              ...
              <dd className="roles-list--dd">
                {gettext(
                  'The editor can see everything about the data collected via this investigation'
                )}
              </dd>
              ...
              <dd className="roles-list--dd">
                {gettext(
                  'A viewer can see the data collected via this investigation but is not able to modify; to sort it or to download it'
                )}
              </dd>
            </dl>
          </p>
