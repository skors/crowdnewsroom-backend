...
const actionsWrapper = document.getElementsByClassName(
  'formresponse-details--actions'
)[0]

if (actionsWrapper) {
  // due to permission rights this element doesn't exist for some users
  const id = actionsWrapper.getAttribute('data-response-id')
  const investigationSlug = actionsWrapper.getAttribute(
    'data-investigation-slug'
  )
